...
with(_inst) {
	stage = "game_files/stage4_postbattle"
	file = string(stage) + string(global.suf) + ".ini";
	ini_open(file);
	switch (global.character) {
	case "REIMU":
    scr_dialogue_line(0,spr_reimu_happy,spr_marisa_anger,snd_blip_m,false);
	scr_dialogue_line(1,spr_reimu_happy,spr_marisa_anger,snd_blip_f,false);
	scr_dialogue_line(2,spr_reimu_content,spr_marisa_anger,snd_blip_m,false);
    text_last = 2;
	for (var i = 0; i < 3; i += 1) {
	...
	scr_dialogue_line(1,spr_marisa_content,spr_reimu_anger,snd_blip_f,false);
	scr_dialogue_line(2,spr_marisa_happy,spr_reimu_anger,snd_blip_m,false);
    text_last = 2;
	for (var i = 0; i < 3; i += 1) {
		text[i] = string_replace_all(text[i], "#", "\n");
	}
